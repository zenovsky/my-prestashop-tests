...
class CustomersAPI:
    ...
    def get(self, customer_id: int):
        response = self.client.request(method="GET", endpoint=f"{self.RESOURCE}/{customer_id}")
    ...
    def update(self, customer_id: int, xml: str) -> int:
        response = self.client.request(method="PUT", endpoint=f"{self.RESOURCE}/{customer_id}", data=xml)
        if response.status_code != 200:
            raise AssertionError(
                f"Failed to update customer {customer_id}: {response.status_code} {response.text[:200]}"
            )
    ...
    def delete(self, customer_id: int):
        response = self.client.request(method="DELETE", endpoint=f"{self.RESOURCE}/{customer_id}")
        if response.status_code != 200:
    ...
    def update_raw(self, customer_id, xml):
        return self.client.request(method="PUT", endpoint=f"{self.RESOURCE}/{customer_id}", data=xml)

    @log_action
    def get_raw(self, customer_id):
        return self.client.request(method="GET", endpoint=f"{self.RESOURCE}/{customer_id}")
