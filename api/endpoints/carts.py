...
class CartsAPI:
    ...
    def get(self, cart_id: int):
        response = self.client.request(method="GET", endpoint=f"{self.RESOURCE}/{cart_id}")
    ...
    def update(self, cart_id: int, xml: str) -> int:
        response = self.client.request(method="PUT", endpoint=f"{self.RESOURCE}/{cart_id}", data=xml)
        if response.status_code != 200:
    ...
    def delete(self, cart_id: int):
        response = self.client.request(method="DELETE", endpoint=f"{self.RESOURCE}/{cart_id}")
        if response.status_code != 200:
