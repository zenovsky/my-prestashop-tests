...
class OrdersAPI:
    ...
    def get(self, order_id: int):
        response = self.client.request(method="GET", endpoint=f"{self.RESOURCE}/{order_id}")
    ...
    def update(self, order_id: int, xml: str) -> int:
        response = self.client.request(method="PUT", endpoint=f"{self.RESOURCE}/{order_id}", data=xml)
        if response.status_code != 200:
    ...
    def delete(self, order_id: int):
        response = self.client.request(method="DELETE", endpoint=f"{self.RESOURCE}/{order_id}")
        if response.status_code != 200:
