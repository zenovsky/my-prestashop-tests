...
class OrderFactory:
    def __init__(self, orders_api, customer_factory, address_factory, cart_factory):
        self.api = orders_api
    ...
    def create(self):
        ...
        cart_data = self.cart_factory.create(customer_id=customer_id, address_id=address_id)
        cart_id = cart_data["cart_id"]

        xml = OrderBuilder.create(
            customer_id=customer_id, secure_key=secure_key, address_id=address_id, cart_id=cart_id
        )
        order_id = self.api.create(xml)
        self._ids.append(order_id)
        return {"order_id": order_id, "customer_id": customer_id, "address_id": address_id, "cart_id": cart_id}
    ...
    def update(self, order_id, cart_id, customer_id, address_id, new_note):
        xml = OrderBuilder.update(
            order_id=order_id, cart_id=cart_id, customer_id=customer_id, address_id=address_id, note=new_note
        )
