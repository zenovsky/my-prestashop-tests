...
class BasePage:
    ...
    def wait_title(self, title, timeout=6):
        try:
            ...
            raise AssertionError(f"Wait title '{title}', but got '{self.driver.title}'")
    ...
    def wait_element(self, selector, timeout=10, by=By.CSS_SELECTOR):
        try:
            ...
            raise AssertionError(f"Didn't wait for the element to become visible: {selector}")
    ...
    def switch_to_frame(self, locator_tuple):
        try:
            ...
            raise AssertionError(f"Couldn't wait for the iframe and couldn't switch: {locator_tuple}")
